...
(function initTMDFinancialSuite() {
  ...
  function buildFinancialSuiteHTML() {
    ...
    return `
      <div id="tmd-financial-suite-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6 overflow-y-auto transition-opacity duration-300" style="background: rgba(4, 6, 12, 0.85) !important; backdrop-filter: blur(24px) saturate(140%) !important; -webkit-backdrop-filter: blur(24px) saturate(140%) !important;">
  ...
  window.tmdPrintFinancialDossier = function() {
    ...
    if (!printWindow) {
      alert('Por favor permita ventanas emergentes para generar el dictamen financiero.');
      return;
    }
